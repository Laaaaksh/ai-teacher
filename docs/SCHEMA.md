...
`documents`/`document_chunks`, the RAG slice fills `document_chunks.embedding`
and `document_outlines`, and the teaching engine (`lib/teach/`) fills the
rest. What is still unclaimed:

- Lesson player / video slice: `advanceLessonSessionScene` and
  `updateLearningPathProgress` exist but have no caller yet — the player is
  what moves `current_scene_order` and unlocks the next path step as a
  learner finishes one.

That needs no schema change for the features described in
docs/ARCHITECTURE.md; if a later slice needs a new column, add a migration
rather than editing an existing table's `CREATE TABLE` in place (see
migration v2 in `lib/db/migrations.ts` for the pattern).
